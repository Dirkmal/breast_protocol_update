...
export class ReportService {
   ...
   */
  private saveReportComponents(
    reportId: string,
    report: Report
  ): Observable<Report> {
    // Create observables for inserting each component
    const initialDetailsObs = from(
      this.supabase.from('initial_details').insert({
        report_id: reportId,
    ...
    // Specimen type
    const specimenTypeObs = from(
      this.supabase.from('specimen_types').insert({
        report_id: reportId,
        core_needle_biopsy: report.macroscopy.specimen_type.core_needle_biopsy,
        wide_local_excision:
          report.macroscopy.specimen_type.wide_local_excision,
        ...
        wide_bore_needle_biopsy:
          report.macroscopy.specimen_type.wide_bore_needle_biopsy,
      })
    );

    // Specimen dimensions
    const specimenDimensionsObs = from(
      this.supabase.from('specimen_dimensions').insert({
        report_id: reportId,
    ...
    // Axillary procedure
    const axillaryProcedureObs = from(
      this.supabase.from('axillary_procedures').insert({
        report_id: reportId,
        no_lymph_node_procedure:
          report.macroscopy.axillary_procedure.no_lymph_node_procedure,
        axillary_node_sample:
          report.macroscopy.axillary_procedure.axillary_node_sample,
        sentinel_node_biopsy:
          report.macroscopy.axillary_procedure.sentinel_node_biopsy,
        axillary_node_clearance:
          report.macroscopy.axillary_procedure.axillary_node_clearance,
        intrammary_node: report.macroscopy.axillary_procedure.intrammary_node,
      })
    );

    // In situ carcinoma
    const inSituCarcinomaObs = from(
      this.supabase.from('in_situ_carcinomas').insert({
        report_id: reportId,
        ductal_carcinoma_in_situ:
          report.microscopy.in_situ_carcinoma.ductal_carcinoma_in_situ,
        lobular_carcinoma_in_situ:
          report.microscopy.in_situ_carcinoma.lobular_carcinoma_in_situ,
    ...
    // Invasive carcinoma
    const invasiveCarcinomaObs = from(
      this.supabase.from('invasive_carcinomas').insert({
        report_id: reportId,
        ic_present: report.microscopy.invasive_carcinoma.ic_present,
        invasive_tumor_size:
          report.microscopy.invasive_carcinoma.invasive_tumor_size,
        ...
        lympho_vascular_invasion:
          report.microscopy.invasive_carcinoma.lympho_vascular_invasion,
        site_of_other_nodes:
          report.microscopy.invasive_carcinoma.site_of_other_nodes,
      })
    );

    // Axillary node
    const axillaryNodeObs = from(
      this.supabase.from('axillary_nodes').insert({
        report_id: reportId,
    ...
    // Margin
    const marginObs = from(
      this.supabase.from('margins').insert({
        report_id: reportId,
        ...
        skeletal_muscle_involvement:
          report.microscopy.margin.skeletal_muscle_involvement,
        surgical_margins: report.microscopy.margin.surgical_margins,
      })
    );

    // Other margins
    const surgicalMarginsActualObs = from(
      this.supabase.from('surgical_margins_actual').insert({
        report_id: reportId,
        ...
        lateral_: report.microscopy.surgical_margins_actual.lateral,
        medial: report.microscopy.surgical_margins_actual.medial,
      })
    );

    // Pathological staging
    const pathologicalStagingObs = from(
      this.supabase.from('pathological_stagings').insert({
        report_id: reportId,
    ...
    // IHC
    const ihcObs = from(
      this.supabase.from('ihcs').insert({
        report_id: reportId,
    ...
    // Pathologist report
    const pathologistReportObs = from(
      this.supabase.from('pathologist_reports').insert({
        report_id: reportId,
        final_diagnosis: report.pathologist_report.final_diagnosis,
        comment: report.pathologist_report.comment,
        consultant_pathologist:
          report.pathologist_report.consultant_pathologist,
  ...
  getReportById(id: string): Observable<Report> {
    ...
    return from(
      this.supabase.from('reports').select('*').eq('id', id).single()
    ).pipe(
      switchMap((result) => {
        if (result.error) {
  ...
  private getMicroscopyData(reportId: string): Observable<Microscopy> {
    ...
    }).pipe(map((result) => result as Microscopy));
  }

  private getIhcData(reportId: string): Observable<IHC> {
    return from(
      this.supabase.from('ihcs').select('*').eq('report_id', reportId).single()
    ).pipe(
      map((result) => {
        if (result.error) {
          throw result.error;
  ...
  private getPathologistReport(
    reportId: string
  ): Observable<PathologistReport> {
    return from(
      this.supabase
